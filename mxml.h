...
XMLContext XMLContextInit(MByteArray** b, XMLFILE** c) {
...
int writeXmlOpenTag(XMLContext* xmlcontext, const char* objectName, size_t name_len, const char* param, size_t param_len, int* level, ...) {
    // indent and open tag <
...
int writeXmlCloseTag(XMLContext* xmlcontext, int* level) {
    *level = *level - 1; 
    for (int i = 0; i < *level; i++)
        *(xmlcontext->xmlfile) = XMLFILEPush(*(xmlcontext->xmlfile), '\t');
    *(xmlcontext->xmlfile) = XMLFILEPush(*(xmlcontext->xmlfile), '<');
...
XMLContext XMLContextFree(XMLContext context) {
...
#define XML(name, ...) for(\
        int UNIQUE_NAME(__i) = writeXmlOpenTag(&ctx__, #name, strlen(#name), #__VA_ARGS__, strlen(#__VA_ARGS__), &__level__ FOREACH(CONFORM__interface__, __VA_ARGS__), NULL);\
            UNIQUE_NAME(__i) == 0;\
            UNIQUE_NAME(__i) = writeXmlCloseTag(&ctx__, &__level__)\
        )
